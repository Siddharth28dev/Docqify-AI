...
DATA_DIR = os.path.join("/tmp", "Docqify") if IS_VERCEL else BASE_DIR
os.makedirs(DATA_DIR, exist_ok=True)
DB_PATH = os.path.join(DATA_DIR, "admin_analytics.db")
...
def html_to_pdf_bytes(html_content, options):
    if WKHTMLTOPDF_PATH:
        config = pdfkit.configuration(wkhtmltopdf=WKHTMLTOPDF_PATH)
    ...
    if pisa is None:
        raise RuntimeError("No PDF engine available. Install wkhtmltopdf or include xhtml2pdf.")

    buffer = io.BytesIO()
    pdf = pisa.CreatePDF(src=html_content, dest=buffer, encoding="utf-8")
    if pdf.err:
        raise RuntimeError("PDF generation failed while rendering HTML.")
...
def load_document_rules_from_env():
    raw = os.environ.get("DOCUMENT_RULES_JSON", "").strip()
    if not raw:
        return {}
    try:
        ...
        app.logger.warning(f"Could not parse DOCUMENT_RULES_JSON: {exc}")
        return {}
...
def apply_env_settings(settings):
    env_settings = {
        ...
        "document_price": os.environ.get("DOCUMENT_PRICE", "").strip(),
        "currency": os.environ.get("PAYMENT_CURRENCY", "").strip(),
    }
    ...
    if os.environ.get("AI_USAGE_ENABLED") is not None:
        settings["ai_usage_enabled"] = env_flag("AI_USAGE_ENABLED", True)

    env_rules = load_document_rules_from_env()
    if env_rules:
        settings["document_rules"] = env_rules
...
def load_settings():
    ...
    settings = apply_env_settings(settings)
...
def download_pdf():
    try:
        ...
        pdf_bytes = html_to_pdf_bytes(data['html_content'], options)
